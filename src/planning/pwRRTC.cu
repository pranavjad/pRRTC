...
namespace pwRRTC {
    ...
    )
    {
        ...
        float *t_radii = radii[t_tree_id] + (tree_idx * MAX_TREE_SIZE);
        unsigned int iter = 0;
        if (threadIdx.x < 4) {
            ...
            if (!config_in_collision) {
                ...
                if (lid == 0) {
                    ...
                    if (d_settings.dynamic_domain) {
                        t_radii[index] = FLT_MAX;
                        if (t_radii[min_index] != FLT_MAX) {
                            t_radii[min_index] *= (1 + d_settings.dd_alpha);
                        }
                    }
                }
                ...
                if (t_radii[min_index] == FLT_MAX) {
                    t_radii[min_index] = min_dist;
                }
                else {
                    t_radii[min_index] = fmaxf(t_radii[min_index] * (1 - d_settings.dd_alpha), d_settings.dd_min_radius);
                }
            }
            if (solved != 0) break;
        }
